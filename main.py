...
gemini_model = "gemini-1.5-pro-latest" # Changed to gemini-1.5-pro for better JSON handling
...
def chat_api():
    ...
    # --- NEW: VALIDATION STEP ---
    is_valid, reason = is_query_valid_for_analysis(user_query)
    if not is_valid:
        # If the query is not valid, return an error message immediately
        def error_generator():
            yield stream_event({'type': 'error', 'content': reason})
...
def is_query_valid_for_analysis(user_query: str) -> (bool, str):
    """
    Checks if the user's query is a valid topic for meta-analysis.
    Returns a tuple of (is_valid, reason).
    """
    print(f"--- Step 0: Validating query: '{user_query}' ---")
    validation_prompt = compose_validation_query(user_query)
    ...
    try:
        ...
        if is_valid:
            ...
            print(f"🔴 Query is invalid. Reason: {reason}")
            
        return is_valid, reason
        
    except Exception as e:
        sentry_sdk.capture_exception(e)
        print(f"An error occurred during query validation: {e}")
        # Default to not valid in case of an error to be safe
        return False, "An internal error occurred while validating your query."
...
def compose_validation_query(user_query: str) -> str:
    """Creates a prompt to validate if a query is suitable for meta-analysis."""
    return (
        "You are a gatekeeper for a research analysis tool. Your task is to determine if a user's query is a legitimate topic for a meta-analysis of academic literature in fields like education, psychology, or social sciences. "
        "Legitimate topics are specific research questions (e.g., 'the effect of phonics instruction on reading comprehension'). "
        "Invalid topics are general knowledge questions, requests for definitions, or nonsensical phrases (e.g., 'What is a chair?', 'the history of the color blue').\n\n"
        f"User query: \"{user_query}\"\n\n"
        "Respond with a single, valid JSON object with two keys: "
        '1. `is_valid` (boolean): true if the query is a legitimate research topic, false otherwise. '
        '2. `reason` (string): A brief explanation for your decision, especially if invalid.'
    )
